...
func (td *TimeDrawer) Update(int) {
	if td.lastPercent != td.Percent {
		...
		td.Sun.W = td.H/2;td.Sun.H = td.H/2;td.Moon.W = td.H/2;td.Moon.H = td.H/2
		padding := td.Sun.W/2
...
func (td *TimeDrawer) Draw(screen *ebiten.Image) {
	td.Back.Draw(screen)
	if td.showSun {td.Sun.Draw(screen)}
	if td.showMoon {td.Moon.Draw(screen)}
	td.Front.Draw(screen)
}
func (td *TimeDrawer) Init(screen *ebiten.Image, data interface{}) (UpdateFunc, DrawFunc) {return td.Update, td.Draw}
