...
func SetWorldStructFromBytes(p *WorldStructure, data []byte) error {
	...
	if !ok {return fmt.Errorf(ERROR_WRONG_WORLD_VERSION, version)}
	fnc(data, p)
	return nil
}
var WorldStructureLoader = map[byte]func([]byte, *WorldStructure)(error) {
	1:func(data []byte, p *WorldStructure)(error) {
		...
		p.SetDisplayWH(int(p.TileMat.Focus().Bounds().X), int(p.TileMat.Focus().Bounds().Y))
...
func (p *WorldStructure) TileMatToBytes() []byte {
	bs, err := p.TileMat.Compress();CheckErr(err)
	return bs
}
...
func (p *WorldStructure) RegionMatToBytes() []byte {
	bs, err := p.RegionMat.Compress();CheckErr(err)
	return bs
}
...
func (p *WorldStructure) StatsToBytes() (mdxBs,mdyBs,maxBs,timBs []byte) {
	...
	var err error
	timBs, err = p.CurrentTime.MarshalBinary();CheckErr(err)
	return
}
